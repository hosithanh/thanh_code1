...
export default function ChitietDulieu(): JSX.Element {
                ...
                .then((res) => {
                    if (res.data.errorCode === SUCCESS) {
                        ...
                        if (loaiTichHop === 'IMPORT_THANHPHANHSMCDT') {
                            ...
                            dataResponse = { errorCode: 0, message: 'Thành công' }
                        }
                    } else {
                        Notification({ status: 'error', message: res.data?.message ?? errorMessage })
                        dataResponse = { errorCode: 99, message: 'Lỗi' }
                    }
                })
                .catch((err) => {
        ...
        window.addEventListener('message', function (e) {
            if (e.data !== undefined) {
                try {
                    ...
                    console.log('Không phải json data')
                }
            }
        })
    ...
    return maDoiTuong && resultDetail && isExist ? (
        <div className='dulieu-detail-iframe' style={{ padding: '0 10px', height: '100%', overflow: 'auto' }}>
            <PageHeader title={`${resultDetail?.ten}${isAdd ? ' - Thêm mới' : ''}`} />
        ...
        <h2 style={{ color: 'red', padding: '10px 10px' }}>
            Mã loại kết quả không tồn tại trên hệ thống số hoá. Vui lòng liên hệ Trung tâm Công nghệ thông tin và Truyền
            thông (0292 3762333) để cấu hình thêm!
        </h2>
    )
}
