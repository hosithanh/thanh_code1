...
import {
    DONVI_LIST_USER_URL,
    DOWNLOAD_FILE_EXCEL_BAOCAOCHITIETKETQUASOHOAMAU2,
    DYNAMIC_URL
...
import {
    ...
    isNullOrUndefined,
    isStringEmpty
} from '../../../common/utils/empty-util'
import { AppState } from '../../../store/interface'

export default function BaoCaoChiTietKetQuaSoHoaMau2(): JSX.Element {
    const { Option } = Select
    ...
    // const [isLoadingMoreDoiTuong, setIsLoadingMoreDoiTuong] = useState(false)
    ...
    const downloadFile = (url, ngayBatDau, ngayKetThuc, donviId, madoituong, name) => {
        axios({
            url: `${url}?1=1${!isNullOrUndefined(madoituong) ? `&madoituong=${madoituong}` : ''}${
    ...
    const fileExcel = (): void => {
        ...
        downloadFile(
            ...
            donviId,
            maDoituong,
            `Baocaochitietketquasohoamau2.xlsx`
        )
    }
    ...
    const onSearchDoituong = (dataSearch) => {
        ...
        typingRef.current = setTimeout(() => {
            axios
                .get(
                    `${DYNAMIC_URL}/doituong/all?searchData=${dataSearch}${
                        !isNullOrUndefined(donviId) ? `&donVi=${donviId}` : ''
                    }`,
                    Authorization()
                )
    ...
    const onScrollDoiTuong = (e) => {
        if (!isLoadingMoreDoiTuongStop && e.target.scrollTop + e.target.offsetHeight === e.target.scrollHeight) {
            ...
            axios
                .get(
                    `${DYNAMIC_URL}/doituong/all?curPage=${currPage}${
                        !isNullOrUndefined(donviId) ? `&donVi=${donviId}` : ''
                    }`,
                    Authorization()
                )
    ...
    return (
        <Fragment>
            <div className='group-action'>
                <PageHeader ghost={false} title='Báo cáo chi tiết kết quả số hóa mẫu 2' extra={[]} />
